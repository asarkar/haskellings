...
main = do
  ...
  D.getCurrentDirectory >>= putStrLn . P.printf "Running from directory: %s"
  D.getHomeDirectory >>= putStrLn . P.printf "Home directory is: %s"
  D.getHomeDirectory
    >>= ((length <$>) . D.listDirectory)
    >>= putStrLn . P.printf "Home directory contains %d sub-paths."
